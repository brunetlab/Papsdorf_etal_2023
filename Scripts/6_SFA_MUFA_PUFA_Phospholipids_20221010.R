...
# Approach: 1) Fatty acyl chain abundance in membrane lipids (individual fatty acyl concentration matches the corresponding lipid). Odd chain fatty acids and fatty acids with d, O and t modifications are not analyzed.
...
# Output:  Pdf: 6_MUFAtoPUFA_Phospholipids.pdf
#          Csv: 6_SFA_MUFA_PUFA_Phospholipids.csv   
...
########Filter Phopsholipid classes

final_table_1<-final_table%>%filter(grepl("PE",Class)) #105 PE lipids identified
final_table_2<-final_table%>%filter(grepl("PC",Class)) #55 PE lipids identified
final_table_3<-final_table%>%filter(grepl("PI",Class)) #34 PI lipids identified
final_table_4<-final_table%>%filter(grepl("PS",Class)) #37 PS lipids identified
final_table_5<-final_table%>%filter(grepl("PG",Class)) #2 PG lipids identified

final_table_P<-rbind(final_table_1,final_table_2,final_table_3,final_table_4,final_table_5)
...
FA_table_1<-bind_rows(FA_table,FA_temp) #merge

colnames(FA_table_1)[1]<-c("FA")
...
#Save
write.csv(SMP_table, '../Output_Data/6_SFA_MUFA_PUFA_Phospholipids.csv')
...
plot<- filter(ptSMP_table, grepl("MUFAtoPUFA",Var)) #subset MUFA to PUFA


######## plot MUFA to PUFA

SMP<-ggplot(plot,aes(x = Group, y = Val, fill = Group)) +
  geom_boxplot(outlier.shape = NA)+
  xlab("")+
  ylab("MUFAtoPUFA ratio phospholipids")+  #changed
...
ggsave("../Output_Figures/6_MUFAtoPUFA_Phospholipids.pdf", SMP, width=0.8, height=1.5, units="in", scale=3) #changed filename
